...
> = (set) => ({
  ...
  hitsCount: 0,
  increaseHitsCount: (hitTime: number) =>
    set((state) => {
  ...
  missesCount: 0,
  increaseMissesCount: () =>
    set((state) => ({ missesCount: state.missesCount + 1 })),

  averageTiming: null,

  keysPerformance: {},
  increaseKeyHitsCount: (key: string, hitTime: number) =>
    set((state) => {
      const keysPerformance = state.keysPerformance;

      const totalHitTimes = !(key in keysPerformance)
        ? 1
        : state.keysPerformance[key].hitsCount + 1;
  ...
  increaseKeyMissesCount: (key: string) =>
    set((state) => {
      const keysPerformance = state.keysPerformance;

      const totalMissedTimes = !(key in keysPerformance)
        ? 1
        : state.keysPerformance[key].missesCount + 1;
