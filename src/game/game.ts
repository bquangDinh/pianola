...
export class Game extends EventEmitter {
  private midiController: MIDIInputable =
    AppConfigs.midiController === MIDIControllers.PIANO
      ? MidiController
      : MockMidiController;
  ...
  public init() {
    ...
    this.notesManager.on(
      NotesManagerEvents.NOTE_HIT_ENDPOINT,
  ...
  public getGameObjectsByClass(c: typeof GameObject) {
    return this.gameObjects.filter(
      (obj) => Object.getPrototypeOf(obj).constructor === c
    );
  }
  ...
  private compareTwoNotes(n1: string, n2: string) {
    ...
    const numN1 = this.getNumericalValueOfNote(
      n1Note,
      n1Accidental,
      Number.parseInt(n1Octave)
    );

    // Calculate the numerical value of note 2
    const numN2 = this.getNumericalValueOfNote(
      n2Note,
      n2Accidental,
      Number.parseInt(n2Octave)
    );

    return numN1 === numN2;
  }

  private getNumericalValueOfNote(
    note: string,
    accidental: string,
    octave: number
  ): number {
